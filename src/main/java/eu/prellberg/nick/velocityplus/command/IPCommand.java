...
public class IPCommand implements SimpleCommand {
    ...
    public IPCommand(ProxyServer proxyServer, VelocityPlus velocityPlus, Logger logger) {
        ...
        manager.register(manager.metaBuilder("ip").build(), this);
    }

    @Override
    public void execute(Invocation invocation) {
        CommandSource source = invocation.source();
        if (!source.hasPermission("vplus.ip")) {
        ...
        if (invocation.arguments().length < 1) {
            source.sendMessage(Component.text(String.join(" ", velocityPlus.getConfig().getString("general.prefix"), velocityPlus.getConfig().getString("ip.command.missingUser"))));
            return;
        }
        ...
        source.sendMessage(Component.text(String.join(" ", velocityPlus.getConfig().getString("general.prefix"), velocityPlus.getConfig().getString("ip.command.result").replaceAll("<<ip>>", String.valueOf(playerFound.get().getRemoteAddress())).replaceAll("<<user>>", playerFound.get().getUsername()))));
    }
